...
for ( i in 1:length(cor_plots_data)) {
  ...
  if (name == "experimental_all_archival") {
    axis_text_x <- element_blank()
    axis_text_y <- element_text()
  } else if (name == "experimental_all_peekbank") {
    axis_text_x <- element_blank()
    axis_text_y <- element_blank()
  } else if (name == "non-experimental_all_archival") {
  ...
  plot <- data %>%
      ...
      scale_fill_gradient2(low = "#276419", midpoint = 0, mid = "white", high = "#8E0152",
                            limits = c(-1, 1),
                            breaks=c(-1,-.5,0,.5,1)) +
    ...
    scale_y_discrete(drop = FALSE) +
      ...
      theme(axis.text.x = axis_text_x,
            ...
            aspect.ratio=1) +
      ggtitle(paste0(str_to_title(str_replace_all(str_replace(str_replace(name, "non-experimental", "familiar"), "_all", ""), "_", ", "))))
...
#Plots
plot_a_list(c(corr_plots_sp), 2, 2) +
  patchwork::plot_layout(guides = 'collect') 
...
pooled_icc_for_plot <- means_cis_icc %>%
  bind_rows() %>%
  ...
  mutate(variable = case_when(variable == "latency_to_switch" ~ "Reaction time",
         ...
         data_split = case_when(data_split == "all" ~ "All initial looks",
                                TRUE ~ snakecase::to_sentence_case(data_split)),
         
         data_split = factor(snakecase::to_sentence_case(data_split), levels = c("Target", "Distractor", "All initial looks")),
