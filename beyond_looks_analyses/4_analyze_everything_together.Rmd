...
for_correlation_with_age <- data_by_subject %>%
  ...
  dplyr::select(-(starts_with("sd")), -(starts_with("n_trials")), -(ends_with("centred")), -subject_id, -ends_with("exp"), -(ends_with("diff"))) %>%
...
better_names <- data.frame(
  old_name = c("age_months", "exp_to_target_lang", "fre_exp", "eng_exp", "mean_prop_looking",
               #"mean_prop_looking_diff", 
               "mean_number_fixations", "mean_prop_shift_target_initial", "mean_prop_shift_distractor_initial", "mean_RT", "mean_aoi_switches", "mean_duration_of_look", "mean_first_look_dur", "mean_longest_look", "mean_peak_pupil_size_difference", "mean_mean_pupil_size_difference", "mean_peak_pupil_latency"),
  new_name = c("Age in months", "Target language exposure", "French exposure", "English exposure", "Proportion looking", 
               #"Proportion looking difference", 
               "Total number of fixations", "Proportion trials staying on target", "Proportion trials shifting to target", "Reaction time", "Mean number of AOI switches", "Mean duration of looks", "Mean duration of first look", "Duration of longest look", "Peak pupil size difference", "Mean pupil size difference", "Peak pupil latency"))

var_order <- c("Age in months", "Proportion looking", "Reaction time",
               #"Proportion looking difference", 
               "Total number of fixations", "Proportion trials staying on target", "Proportion trials shifting to target",  "Mean number of AOI switches", "Mean duration of looks", "Mean duration of first look", "Duration of longest look", "Peak pupil size difference", "Mean pupil size difference", "Peak pupil latency", "Target language exposure")
...
for ( i in 1:length(cor_plots_data)) {
  ...
  if (name == "non-experimental_all_archival") {
    ...
    df_shape <- tibble(group = rep(c("pred_lit", "sampling", "pupil"), each = 4),
                       x = rep(c(.25, -9, -9, .25), 3),
                       y = c(11.6, 11.6, 13.4, 13.4, 4.6, 4.6, 11.4, 11.4, 1.6, 1.6, 4.4, 4.4),
                       alpha = .2)
  ...
  plot <- data %>%
      ...
      geom_point(aes(size = abs(corr), color = corr),
                 shape = 19) +
      ggforce::geom_shape(data = df_shape, aes(x=x, y=y, group = group, fill = group, alpha = alpha), radius = .01, show.legend = FALSE) +
      scale_color_gradient2(low = "#8E0152", midpoint = 0, mid = "white", high = "#276419",
                            limits = c(-1, 1),
                            breaks=c(-1,-.5,0,.5,1)) +
...
for (i in 1:length(cor_matrices_pearson)) {
  ...
  data <- cor_matrices_pearson[[i]] 
  element <- names(cor_matrices_pearson[i])
  if (str_detect(element, "prop_looking_diff")) {
    next
  }
...
possible_columns <- c("prop_looking", "latency_to_switch", 
                      #"prop_looking_diff", 
                      "number_fixations", "correct_shift", "remain_on_target", "total_aoi_switches", "duration_of_look", "first_look_dur", "longest_look_by_trial", "peak_pupil_size_difference", "mean_pupil_size_difference", "peak_pupil_latency")

fct_lvls <- c("Proportion looking", "Reaction time",
              #"Proportion looking difference", 
              "Number fixations", "Proportion trials shifting to target", "Proportion trials staying on target",  "Number of AOI switches", "Mean duration of all looks", "Duration of first look", "Duration of longest look", "Peak pupil size difference", "Mean pupil size difference", "Peak pupil latency")
...
df_shape <- tibble(group = rep(c("pred_lit", "sampling", "pupil"), each = 4),
                       x = rep(c(-.175, -.625, -.625, -.175), 3),
                       y = c(10.6, 10.6, 12.4, 12.4, 3.6, 3.6, 10.4, 10.4, 0.6, 0.6, 3.4, 3.4),
                       alpha = .2)
...
pooled_icc_for_plot %>%
  ...
  ggforce::geom_shape(data = df_shape, aes(x=x, y=y, group = group, fill = group, alpha = alpha), radius = .01, show.legend = FALSE, inherit.aes = FALSE) +
  ...
  scale_fill_manual(values = c("orange", "red", "blue")) +
  scale_alpha_identity() +
  coord_cartesian(xlim = c(-.125, 1), clip = "off") +
...
# Laia added this because my version of sjstats did not have the std_beta() function
#library(remotes)
#devtools::install_version("sjstats", version = "0.17.2")
library(sjstats)
library(lme4)
library(lmerTest)
...
#------------model comparisons
mod_comparisons <- list()

#------------plots
inter_plots <- list() #interaction plots

#-------------------------------------------------------Make models and save to list
...
for (i in 1:length(data_by_subject_long_split)) {
  ...
  if (sjmisc::all_na(data$response)) {
    ...
    if (str_detect(element, "archival")) {
        ...
        #simple vs interaction model comparison
        mod_compare <- anova(simple_mod, inter_mod)
        mod_comparisons[[paste0(element, "_", var_name)]] <- mod_compare
        #interaction plot
...
fct_lvls <- c("Proportion looking", "Reaction time", 
              #"Proportion looking difference", 
              "Number fixations", "Proportion trials shifting to target", "Proportion trials staying on target",  "Number of AOI switches", "Duration of all looks", "Duration of first look", "Duration of longest look", "Peak pupil size difference", "Mean pupil size difference", "Peak pupil latency")

cleaned_plot_data <- summaries_for_plot %>%
  bind_rows() %>%
  filter(!str_detect(term, "Intercept")) %>%
  filter(!str_detect(model, "prop_looking_diff")) %>%
...
df_shape <- tibble(group = rep(c("pred_lit", "sampling", "pupil"), each = 4),
                   initial_look = "Combined",
                       x = rep(c(.4, -2.15, -2.15, .4), 3),
                       y = c(10.6, 10.6, 12.4, 12.4, 3.6, 3.6, 10.4, 10.4, 0.6, 0.6, 3.4, 3.4),
                       alpha = .2)
...
print_list[[paste("Model Comparisons - simple vs. interaction (archival data only)")]] <- mod_comparisons 
...
#Models - Mixed Effects (all variables/datasets)
...
data_by_trial_long_split <- data_by_trial %>%
  #filter(exp_type == "both_types") %>%
  dplyr::select(-prop_looking_diff) %>%
...
for (i in 1:length(data_by_trial_long_split)) {
  ...
  #Skip the loop for variables that area all NA (like the pupil ones in the peekbank data and also the prop_looking_diff variable)
  if (sjmisc::all_na(data$response) | str_detect(element, "prop_looking_diff")) {
    next
  } else {
  ...
  print(paste(element, var_name))
    ...
    if (str_detect(element, "archival")) {
      ...
      #aoi switches get zero inflated poisson
        ...
        simple_lmem_sum <- summary(simple_lmem)
      #number fixations and latency to switch gets regular poisson
      } else if (var_name %in% c("number_fixations", "latency_to_switch")) {
        simple_lmem <- glmmTMB(response ~ age_months + exp_to_target_lang + (1|dataset_name/subject_id),
                                  data = data, family = poisson, control = glmmTMBControl())
        simple_lmem_sum <- summary(simple_lmem)
      #RT, duration of looks, and pupil latency get gamma log link
      } else if (var_name %in% c("duration_of_look", "longest_look_by_trial", "peak_pupil_latency")) {
        simple_lmem <- glmmTMB(response ~ age_months + exp_to_target_lang + (1|dataset_name/subject_id),
                                  data = data, family = Gamma(link = "log"), control = glmmTMBControl())
        simple_lmem_sum <- summary(simple_lmem)
      # longest look needs ziGamma to account for overdispersion
      } else if (var_name %in% c("longest_look_by_trial", "first_look_dur")) {
        simple_lmem <- glmmTMB(response ~ age_months + exp_to_target_lang + (1|dataset_name/subject_id),
...
# Check variables with issues and models
...
df_all_both_types_archival_longest_look_by_trial <- as.data.frame(data_by_trial_long_split$all_both_types_archival_longest_look_by_trial) 
...
modGauss_dist_arch_longest_look <- glmmTMB(response ~ age_months + exp_to_target_lang + (1|dataset_name/subject_id),
                                  data = df_distractor_experimental_archival_longest_look_by_trial, 
                                 family = gaussian,
                                 control = glmmTMBControl())

summary(modGauss_dist_arch_longest_look)

# Test model
alk <- DHARMa::simulateResiduals(modGamm_dist_arch_longest_look, plot = T)
...
non_exp_lmem_summaries <- model_summaries_lmem[str_detect(names(models_lmem), "non-exp")]

lmem_summaries_for_plot <- list()

for (i in 1:length(non_exp_lmem_summaries)) {
  element <- names(non_exp_lmem_summaries[i])
  summaries <- non_exp_lmem_summaries[[i]]$simple
  for (j in 1:length(summaries)) {
    summary <- summaries[[j]]
    variable <- names(summaries[j])
    summary_tidy <- as_tibble(summary$coefficients$cond, rownames = "term") %>% 
      janitor::clean_names() %>% 
      mutate(pr_z = round(pr_z, digits = 6),
             model = paste(element, variable, sep = "_"))
    lmem_summaries_for_plot[paste0(element, "_", variable)] <- list(summary_tidy)
  }
}
...
cleaned_plot_data <- lmem_summaries_for_plot %>%
  bind_rows() %>%
  filter(!str_detect(term, "Intercept")) %>%
  filter(!str_detect(model, "prop_looking_diff")) %>%
  mutate(initial_look = str_extract(model, ".*(?=_non)"),
  ...
  mutate(variable = case_when(variable == "latency_to_switch" ~ "Reaction time",
                              ...
                              variable == "prop_looking_diff" ~ "Proportion looking difference",
                              variable == "total_aoi_switches" ~ "Number of AOI switches",
                              variable == "pupil_size_difference" ~ "Mean pupil size difference",
                              TRUE ~ snakecase::to_sentence_case(variable)),
         initial_look = case_when(initial_look == "all" ~ "All initial looks",
                                TRUE ~ snakecase::to_sentence_case(initial_look)),
...
cleaned_plot_data %>%
  ...
  mutate(initial_look = case_when(initial_look == "All initial looks" ~ "Combined",
                                  TRUE ~ initial_look)) %>%
  ggplot(aes(x = term, y = fct_rev(var_ordered), fill = pr_z)) +
  geom_tile(color = "black") +
  #geom_text(aes(label = round(pr_t, digits = 3))) +
  #geom_text(aes(label = paste('Beta', "==", round(standardized, digits = 3))), parse = TRUE) +
  ...
  guides(
    #reverse color order (higher value on top)
    fill = guide_colorbar(reverse = TRUE)) +
  ggtitle("Standardized coefficients for mixed-effects model estimates")

ggsave(here("figures/lmem_betas_plot.png"), width = 9, height = 6, bg = "white")

```
#Models - Mixed Effects (prop_looking vs looking difference)
...
#use trial-level data for lmems
data_by_trial_long_propdiff <- data_by_trial %>%
  #filter(exp_type == "both_types") %>%
  dplyr::select(data_split, data_source, exp_type, dataset_name, subject_id, age_months, age_months_centred, exp_to_target_lang, eng_exp, fre_exp, target_word, prop_looking, prop_looking_diff) %>%
  pivot_longer(12:ncol(.), names_to = "variable", values_to = "response") %>%
  #scale the predictor variables to standardize before doing the models
  mutate(exp_to_target_lang = exp_to_target_lang/100) %>%
  group_by(data_split, exp_type, data_source, variable) %>%
...
# Check which datsets contribute data to prop looking and prop diff
datasets_with_data <- data_by_trial_long_propdiff %>% 
  group_by(data_split, data_source, exp_type, variable, dataset_name) %>% 
  summarise(response = mean(response, na.rm = T)) 

# check how many datasets contribute to each measure
datasets_with_data_count <- datasets_with_data %>% 
  group_by(data_split, data_source, exp_type, variable) %>%
  mutate(count = as.numeric(case_when(!is.na(response) ~ 1,
                           is.na(response) ~ 0))) %>% 
  summarise(total = sum(count))

# Make list
data_by_trial_long_split_propdiff <- data_by_trial_long_propdiff%>%
  split(f = paste0(.$data_split, "_", .$exp_type, "_", .$data_source, "_", .$variable))

#Model the data

list_levels_1_lmem <- unique(str_remove(names(data_by_trial_long_split_propdiff), "(?<=peekbank|archival).*"))
...
for (i in 1:length(data_by_trial_long_split_propdiff)) {
  data <- data_by_trial_long_split_propdiff[[i]] 
  element <- str_remove(names(data_by_trial_long_split_propdiff[i]), "(?<=peekbank|archival).*")
  var_name <- unique(data$variable)
  ...
  print(paste(element, var_name))
    ...
    if (str_detect(element, "archival")) {
      ...
      #--------------------------------------------------------------------Simple model
      #prop looking variables and mean/peak pupil get regular lmer
      if(var_name %in% c("prop_looking", "prop_looking_diff")) {
        simple_lmem <- glmmTMB(response ~ age_months + exp_to_target_lang + (1|dataset_name/subject_id),
                                  data = data, family = gaussian())
        simple_lmem_sum <- summary(simple_lmem)
      }
...
for (i in 1:length(models_lmem)) {
  element <- names(models_lmem[i])
  models <- models_lmem[[i]]$simple
  for (j in 1:length(models)) {
    model <- models[[j]]
    variable <- names(models[j])
    summary_tidy <- as.tibble(broom.mixed::tidy(model, effects = "fixed")) %>% 
      janitor::clean_names() %>% 
      mutate(across(where(is.numeric), round, digits = 6),
             model = paste(element, variable, sep = "_"))
    results_lmem[paste0(element, "_", variable)] <- list(summary_tidy)
  }
}

fct_lvls2 <- c("Proportion looking", "Proportion looking difference")

cleaned_results_lmem <- results_lmem %>%
  bind_rows() %>%
  filter(!str_detect(term, "Intercept")) %>% 
  mutate(#initial_look = str_extract(model, ".*(?=_non)"), # used to filter all non-experimental trials?
        initial_look = str_extract(model, ".*?(?=_)"),
         dataset = str_extract(model, "archival|peekbank"),
         variable = str_extract(model, "(?<=archival_|peekbank_).*"),
         variable = str_remove(variable, "mean_"),
         term = case_when(str_detect(term, "age_") ~ "Age",
        ...
        # remove the str_replace here to de-anonymize
         dataset = str_replace(str_to_title(dataset), "Archival", "ANONYM")) %>% 
  mutate(variable = case_when(variable == "prop_looking" ~ "Proportion looking",
                              variable == "prop_looking_diff" ~ "Proportion looking difference",
                              TRUE ~ snakecase::to_sentence_case(variable)),
         initial_look = case_when(initial_look == "all" ~ "All initial looks",
                                TRUE ~ snakecase::to_sentence_case(initial_look)),
         data_split = factor(snakecase::to_sentence_case(initial_look), levels = c("Target", "Distractor", "All initial looks")),
         var_ordered = factor(variable, levels = fct_lvls2, ordered = T))
...
fct_lvls <- c("Proportion looking", "Reaction time",
              #"Proportion looking difference", 
              "Number fixations", "Proportion trials shifting to target", "Proportion trials staying on target",  "Mean number of AOI switches", "Mean duration of all looks", "Duration of first look", "Duration of longest look", "Peak pupil size difference", "Mean pupil size difference", "Peak pupil latency")
...
smds_for_plot <- means_cis %>%
  ...
  mutate(variable = case_when(variable == "RT" ~ "Reaction time",
         ...
         data_split = factor(snakecase::to_sentence_case(data_split), levels = c("Target", "Distractor", "Combined")),
         var_ordered = factor(variable, levels = fct_lvls, ordered = T))%>%
  filter(!(data_split == "Combined" & str_detect(variable, "staying|shifting|Reaction")))

df_shape <- tibble(group = rep(c("pred_lit", "sampling", "pupil"), each = 4),
                       x = rep(c(-1.875, -1.075, -1.075, -1.875), 3),
                       y = c(10.6, 10.6, 12.4, 12.4, 3.6, 3.6, 10.4, 10.4, .6, .6, 3.4, 3.4),
                       alpha = .2)
smds_for_plot %>%
  #remove "all initial looks" for target and distractor only types
