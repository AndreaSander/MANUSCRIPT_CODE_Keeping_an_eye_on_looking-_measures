...
library(remotes)
install_version("sjstats", version = "0.17.2")
library(sjstats)
library(lme4)
library(lmerTest)
...
for (i in 1:length(data_by_subject_long_split)) {
  ...
  if (sjmisc::all_na(data$response)) {
    ...
    if (str_detect(element, "archival")) {
        ...
        #LMEM LAIA: the site/block does not work for me (can you check Hilary?)
...
for (i in 1:length(data_by_trial_long_split)) {
  ...
  print(paste(element, var_name))
    ...
    if (str_detect(element, "archival")) {
      ...
      } else if (var_name %in% c("duration_of_look", "first_look_dur", "longest_look_by_trial", "peak_pupil_latency")) {
        simple_lmem <- glmmTMB(response ~ age_months + exp_to_target_lang + (1|dataset_name/subject_id),
                                  data = data, family = Gamma(link = "log"), control = glmmTMBControl())
        simple_lmem_sum <- summary(simple_lmem)
      # longest look needs ziGamma to account for overdispersion
      } else if (var_name %in% c("longest_look_by_trial")) {
        simple_lmem <- glmmTMB(response ~ age_months + exp_to_target_lang + (1|dataset_name/subject_id),
...
modGamm_dist_arch_longest_look <- glmmTMB(response ~ age_months + exp_to_target_lang + (1|dataset_name/subject_id),
                                  data = df_all_both_types_archival_longest_look_by_trial, 
...
datasets_with_data <- data_by_trial_long_propdiff %>% 
  ...
  summarise(response = mean(response, rm.na = T)) 
...
# check how many datasets in each variable
# Prop Looking
summary_archival_prop <- as.data.frame(data_by_trial_long_split_propdiff$all_both_types_archival_prop_looking) %>% 
  group_by(dataset_name) %>% 
  summarise(response = mean(response, rm.na = T)) 

datasets_with_data <- list()
results_lmem <- list()

for (i in 1:length(data_by_trial_long_split_propdiff)) {
  element <- names(data_by_trial_long_split_propdiff[i])
 # models <- data_by_trial_long_split_propdiff[[i]]$simple
  for (j in 1:length(element)) {
    element <- element[[j]]
    variable <- names(element[j]) %>% 
    # summary_tidy <- as.tibble(broom.mixed::tidy(model, effects = "fixed")) %>% 
      # janitor::clean_names() %>% 
      # mutate(across(where(is.numeric), round, digits = 6),
             model = paste(element, variable, sep = "_"))
    datasets_with_data[paste0(element, "_", variable)] <- list(summary_tidy)
  }
}

# Optional: If you want to see a quick overview
data_by_trial_long_split_propdiff %>%
  group_by(data_by_trial_long_split_propdiff) %>%
  summarise(
    total_datasets = n_distinct(dataset_name),
