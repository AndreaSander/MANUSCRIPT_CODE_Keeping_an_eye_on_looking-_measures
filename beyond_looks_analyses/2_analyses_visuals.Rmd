...
prop_shifts_distractor_initial_by_subject <- prop_shifts_distractor_initial_by_trial %>%
  group_by(recording_name) %>%
    summarise(prop_shift_distractor_initial = sum(correct_shift == TRUE)/n(),
              sd_prop_shift_distractor_initial = sd(as.numeric(correct_shift))) #some SDs are NA because the child only has one row, and no standard deviation can be calculated from one value
...
prop_shifts_target_initial_by_subject <- prop_shifts_target_initial_by_trial %>%
  group_by(recording_name) %>%
    summarise(mean_prop_shift_target_initial = sum(correct_shift == TRUE)/n(),
              sd_prop_shift_target_initial = sd(as.numeric(correct_shift))) #some SDs are NA because the child only has one row, and no standard deviation can be calculated from one value
...
peak_pupil_latency_by_subject <- peak_pupil_latency_by_trial %>%
  ...
  summarise(mean_peak_pupil_latency = mean(peak_pupil_latency),
            sd_peak_pupil_latencey = sd(peak_pupil_latency))
...
ICC(icc_aoi_switches , missing = FALSE, lmer = TRUE, alpha=.05)
