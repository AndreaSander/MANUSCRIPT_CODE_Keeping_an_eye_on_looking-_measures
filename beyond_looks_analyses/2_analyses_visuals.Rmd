...
eyetracking_r_data <- kbh_w_items %>%
  ...
  ungroup()%>%
  make_eyetrackingr_data(participant_column = "recording_name",
                       trial_column = "trial_unique",
                       time_column = "timestamp_zero_interest_period",
                       trackloss_column = "trackloss",
                       aoi_columns = c('target','distractor'),
                       treat_non_aoi_looks_as_missing = FALSE
                ) %>%
  mutate(where_looking = as.factor(case_when(target == TRUE ~ "target",
                                             distractor == TRUE ~ "distractor",
                                             TRUE ~ "neither")))

prop_fixation_duration <- eyetracking_r_data  %>%
  rename(aa_target = target,
         bb_distractor = distractor) %>%
  make_time_sequence_data(time_bin_size = 200, 
                          #predictor_columns = "where_looking",
                          aois = c("aa_target", "bb_distractor"),
                          summarize_by = "recording_name" )
#check plot
prop_fixation_duration %>%
  ggplot(aes(x = TimeBin, y = Prop, color = AOI)) +
  geom_smooth() 

what_happens <- analyze_time_bins(data = prop_fixation_duration, predictor_column = "AOI", test = "t.test", p_adjust_method = "bonferroni", alpha = .05)
