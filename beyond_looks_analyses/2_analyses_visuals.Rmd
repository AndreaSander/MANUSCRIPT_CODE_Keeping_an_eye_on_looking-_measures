...
cohens_ds <- experimental_means_sds_long %>%
 ...
 no_nan<- cohens_ds %>%
filter(if_any(cohens_d, ~ !is.nan(.x))) %>%
   filter(!cohens_d==-Inf) #ASM note: we need to check why some where -Infinite.

 
 d_prop_looking<- no_nan%>% filter(variable=="prop_looking")
 
  d_prop_looking_diff<- no_nan%>% filter(variable=="prop_looking_diff")
  
  d_mean_pupil<- no_nan%>% filter(variable=="mean_pupil_size_difference")
 
 ## proportion looking
 mean(d_prop_looking$cohens_d, na.rm=T) #0.18
 range(d_prop_looking$cohens_d, na.rm=T) #-1.33 - 1.58
 
 ## proportion looking difference
  mean(d_prop_looking_diff$cohens_d, na.rm=T) #1.10
 range(d_prop_looking_diff$cohens_d, na.rm=T) #-6.11 - 92.13
 
  ## mean pupil size difference
  mean(d_mean_pupil$cohens_d, na.rm=T) #.03
 range( d_mean_pupil$cohens_d, na.rm=T) # -1.3 - 1.8
