...
kbh_w_items<- kbh_w_items %>%
  ...
  grepl("Door", media_name)  & grepl("Mix", studio_project_name) ~"door",
  ...
  grepl("Toothbrush", media_name) & studio_project_name== "CompMix-36" & grepl("F", studio_test_name) ~ "brosse a dents",
   ...
   grepl("Sheep", media_name) & studio_project_name== "CompMix-36" & grepl("F", studio_test_name) ~ "mouton",
   ...
   grepl("Cow", media_name) & studio_project_name== "CompMix-36" & grepl("F", studio_test_name) ~ "vache",
    grepl("Bunny", media_name) & studio_project_name== "CompMix-36" & grepl("F", studio_test_name) ~ "lapin",
  
    grepl("Ear", media_name) & studio_project_name== "CompMix-36" & grepl("E", studio_test_name) ~ "oreille",
  grepl("Spoon", media_name) & studio_project_name== "CompMix-36" & grepl("E", studio_test_name) ~ "cuillere",
  grepl("Apple", media_name) & studio_project_name== "CompMix-36" & grepl("E", studio_test_name) ~ "pomme",
  grepl("Toothbrush", media_name) & studio_project_name== "CompMix-36" & grepl("E", studio_test_name) ~ "brosse a dents",
...
prop_looking_diff <- kbh_w_items %>%
  ungroup()%>%
  filter(noun_onset>= 360 & noun_onset <= 3000)%>%
  select(recording_name, media_name, recording_timestamp, yolked_pair, target_word, target, distractor)%>%
  group_by(yolked_pair, target_word, media_name) %>%
    summarise(samples_target=sum(target),
            samples_distractor=sum(distractor))%>%
